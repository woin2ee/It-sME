...
private extension ProfileEditingViewController {
    ...
    func bindViewModel() {
        let input: ProfileEditingViewModel.Input = makeInput()
        ...
        [
            ...
            output.deleteAccountComplete
                .emit(to: deleteAccountCompleteBinding),
        ]
            .forEach { $0.disposed(by: disposeBag) }
    }

    private func makeInput() -> ProfileEditingViewModel.Input {
        return .init(
    ...
    var deleteAccountCompleteBinding: Binder<Void> {
        return .init(self) { viewController, _ in
            ...
            let okAction: UIAlertAction = .init(title: "확인", style: .default) { _ in
                let loginViewController = DIContainer.makeLoginViewController()
                viewController.navigationController?.setViewControllers([loginViewController], animated: false)
            }
...
extension ProfileEditingViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        case educationTableView:
            let removeAction: UIContextualAction = .init(style: .destructive, title: "삭제") { (_, _, completionHandler) in
